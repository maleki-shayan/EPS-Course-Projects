...
def multiples_of_ten(start, stop):
  ...
  for i in range(start,stop+1):
    if i%10==0:
      nums+=1
  x=np.zeros((nums,),dtype=np.float64)
  j=0
  for i in range(start,stop+1):
    if i%10==0:
      x[j]=i
...
def slice_indexing_practice(x):
  ...
  even_rows_odd_col=x[0::2,1::2]
  out=(last_row,third_col,first_two_rows_three_cols,even_rows_odd_col)
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return out
...
def zero_row_min(x):
  ...
  # Replace "pass" statement with your code
  y=x.copy()
  minimumIndices=y.argmin(axis=1)
  y[np.arange(y.shape[0]), minimumIndices] = 0
...
def normalize_columns(x):
  ...
  sigma = np.sqrt(np.sum((x - mu) ** 2, axis=0) / x.shape[0])    
  y = (x - mu) / sigma
  y=y.round()
  #############################################################################
  #                            END OF YOUR CODE                               #
