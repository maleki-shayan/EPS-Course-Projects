...
def preAnalyse(listEmail):
    for i in range(len(listEmail)):
        ...
        words=listEmail[i][0].split()
        words=deleteStopWords(words)
        listEmail[i][0] =' '.join(words)
...
def calculateProbilities(XI, y, test, numOfReals, numOfSpams, vocab_size):
    p_y = None
    p_XI = None
    alpha = 1
...
for email in test:
    ...
    for word in words:
        theWord = get_word(word, BoW_dict)
        finalProb0+=calculateProbilities(theWord, 0, test, numOfReals, numOfSpams,vocab_size)
        finalProb1+=calculateProbilities(theWord, 1, test, numOfReals, numOfSpams,vocab_size)
